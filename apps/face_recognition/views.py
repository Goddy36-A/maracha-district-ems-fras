...
def api_register(request):
    try:
        ...
        log_action(
            ...
            request=request,
            metadata={"sample_count": samples, "employee_id": employee.employee_id},
        )
...
def api_verify(request):
    ...
    try:
        ...
        mode       = body.get("mode", "checkin").lower()   # "checkin" | "checkout"
        ...
        if mode not in ("checkin", "checkout"):
            return JsonResponse({"ok": False, "result": "ERROR", "message": "Invalid mode. Use checkin or checkout."}, status=400)
        ...
        if profile is None:
            ...
            return JsonResponse({
                "ok": False, "result": "NO_MATCH",
                "message": "Face not recognised. Please see the front desk.",
            })
        ...
        if mode == "checkin":
            if existing:
                FaceScanLog.objects.create(
                    employee=employee, result="DUPLICATE",
                    distance=distance, ip_address=ip, note="already checked in",
                )
                return JsonResponse({
                    "ok": False, "result": "DUPLICATE",
                    "employee_name": employee.full_name,
                    "check_in_time": existing.check_in_time.strftime("%H:%M"),
                    "message": f"{employee.first_name} already checked in at {existing.check_in_time.strftime('%H:%M')} today.",
                })

            # Determine PRESENT vs LATE from shift
            status = "PRESENT"
            shift  = getattr(employee, "shift", None)
            if shift and hasattr(shift, "start_time") and shift.start_time:
                grace  = datetime.timedelta(minutes=getattr(shift, "grace_minutes", 15))
                cutoff = (datetime.datetime.combine(today, shift.start_time) + grace).time()
                if now > cutoff:
                    status = "LATE"
            ...
            FaceScanLog.objects.create(
                employee=employee, result="MATCHED",
                distance=distance, ip_address=ip, note="check-in",
            )
            return JsonResponse({
                "ok": True, "result": "CHECKIN",
                "employee_id":   employee.employee_id,
                "employee_name": employee.full_name,
                ...
                "message":       (
                    ...
                    f"Check-in recorded at {now.strftime('%H:%M')}."
                    + (" You are late." if status == "LATE" else "")
                ),
            })
        ...
        # ── CHECK-OUT ─────────────────────────────────────────────────────────
            ...
            if not existing:
                FaceScanLog.objects.create(
                    employee=employee, result="ERROR",
                    distance=distance, ip_address=ip, note="checkout without checkin",
                )
                return JsonResponse({
                    "ok": False, "result": "NO_CHECKIN",
                    "employee_name": employee.full_name,
                    "message": f"{employee.first_name} has no check-in record for today. Please check in first.",
                })

            if existing.check_out_time is not None:
                FaceScanLog.objects.create(
                    employee=employee, result="DUPLICATE",
                    distance=distance, ip_address=ip, note="already checked out",
                )
                return JsonResponse({
                    "ok": False, "result": "DUPLICATE",
                    "employee_name": employee.full_name,
                    "check_out_time": existing.check_out_time.strftime("%H:%M"),
                    "message": f"{employee.first_name} already checked out at {existing.check_out_time.strftime('%H:%M')} today.",
                })
            ...
            FaceScanLog.objects.create(
                employee=employee, result="MATCHED",
                distance=distance, ip_address=ip, note="check-out",
            )

            # Calculate hours worked
            ci = datetime.datetime.combine(today, existing.check_in_time)
            co = datetime.datetime.combine(today, now)
            ...
            return JsonResponse({
                ...
                "hours_worked":   hours,
                "distance":       distance,
                "message":        f"Good bye, {employee.first_name}! Check-out at {now.strftime('%H:%M')} — {hours}h worked today.",
            })
